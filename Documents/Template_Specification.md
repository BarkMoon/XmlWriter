...
Excelの1行目のヘッダ名において、以下の特殊な記法が利用可能です。

*   **`#` で始まる列の無視**
    *   ヘッダ名の先頭に `#` を付けると、その列はコメント扱いとなり、XML、コード生成、スクリプト生成のすべてにおいて無視されます。
    *   例: `#Memo`, `#UnusedParam`

*   **テーブル参照と埋め込み**
    *   `PropName:RefTableName(KeyColumn)[]` の形式で記述することで、別のテーブルのデータを検索し、リストとして埋め込むことができます。
    *   **構文**: `プロパティ名:参照テーブル名(参照キー列名)[]`
    *   **動作**:
        1.  対象セルの値（カンマ区切り）をキーとして読み取ります。
        2.  `参照テーブル名` の `参照キー列名` 列を検索します。
        3.  一致した行のデータを、`プロパティ名` の配下にXMLとして展開します（C#クラス生成時は `List<RefTableName>` 型として扱われます）。
    *   例: `Abilities:AbilityTable(Name)[]`
        *   セルの値が `Fire, Ice` の場合、`AbilityTable` から `Name` が `Fire` および `Ice` の行を探し、そのデータを `Abilities` タグの下に展開します。
